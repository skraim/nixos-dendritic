{ inputs, ... }: {
  perSystem = { pkgs, ... }:
    let
      folderRules = pkgs.writeTextDir "main.lua" ''
    ...
    in {
      packages.yazi = inputs.wrapper-modules.wrappers.yazi.wrap {
              ...
              prepend_keymap = [
                {
                  ...
                  desc = "Copy the selected file as a URI";
                }
        ...
        buildCommand.runtimeTheme = {
          after = [ "constructFiles" ];
          data = ''
            rm -f "$out/yazi-config/theme.toml"
            ln -s /home/artem/.config/yazi/theme.toml "$out/yazi-config/theme.toml"
          '';
        };
      };
    };
